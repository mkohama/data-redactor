...
あらかじめ kb-mcp を HTTP サーバとして起動しておきます。
...
1. メインエリアの「kb-mcp サーバー URL」を確認します (既定 `http://localhost:8000/mcp`)。
2. **[文書リストを取得]** を押します。
...
> **処理中は画面の操作ができません。**
> 読み込み・NER検出・LLM検出・マージ&確信度の実行中は、モードやステージの切替バー、
> 各ボタンがグレーになります (終わると元に戻ります)。
> 途中で切り替えると走っている処理が打ち切られてしまうため、意図的にそうしています。
> 処理の待ち時間は文書の大きさによります (LLM 検出は特に時間がかかります)。
