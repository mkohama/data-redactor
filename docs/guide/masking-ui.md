...
- [付録A: NER ビューア（参考ツール）](#付録a-ner-ビューア参考ツール)
...
| **サイドバー**（左） | ⚙️ 設定（モデル・辞書ファイル・平文化 など）。参考ツールのトグルもここ |
...
> **Note:** サイドバーの「🔍 NER ビューア（参考）」を ON にすると、固有表現だけを確認する
> 参考ツールに切り替わります（[付録A](#付録a-ner-ビューア参考ツール)）。通常はマスキングを使います。
...
1. **🔒 マスキング** モードを選ぶ。
2. サイドバーで設定を確認する（モデル・辞書・除外リスト・平文化）。
...
|---|---|
| **カテゴリ** | 社名 / 商標 / 人名 |
| **代表表記** | 正式な表記（伏せ字ラベルの基準） |
| **別名** | 表記ゆれ・略称（カンマ区切り） |
| **置換** | 伏せ字の固定値（空なら `[社1]` 等を自動採番） |
| **部分一致** | 他の語の中でも拾うか（下記） |
...
サイドバーの「🔍 NER ビューア（参考）」を ON にすると、
GiNZA の固有表現抽出だけを色付き表示する参考ツールになります。

![NER ビューア](images/ner-viewer.png)

- 主機能はマスキングです。これは検出結果を眺めるための補助です。
- サイドバーでモデルや表示（高さ・文字サイズ）を調整できます。
- OFF にするとマスキングに戻ります。
