...
対象の絞り込み（2 段）:
  ...
  2. 残った略語を LLM で判定: **秘匿すべき固有の略語か** を見る。
     - 要マスク: ①自社固有の名前（機能名/製品名/コードネーム/独自方式）を指す略語／
       ②固有名詞（会社名・現地法人名・組織名・部署名・人名）を指す略語。
       ※会社名・現地法人名は「製品名でない」ことを理由に外さない（組織を特定する固有名詞＝対象）。
...
各略語について、それが **秘匿すべき固有の略語**（自社固有の名前、または特定の組織・人を指す
固有名詞）か を判定してください。判定材料: 略語の表記、英語名称（＝略語の展開・正式綴り）、
日本語名称、種類、概要・メモ。

【要マスク(yes)にするもの】
A. 自社固有の名前を指す略語: 機能名・製品名・コードネーム・独自方式/アルゴリズム名など、
   外部には通じない自社固有の名前。
B. 固有名詞を指す略語: **会社名・現地法人名・組織名・部署名・人名**（自社・関係会社・取引先を
   問わず、特定の組織や人を指すもの）。
   ★重要: 「製品名ではない」ことを理由に**外さない**。会社名や現地法人名は、組織を特定する
   固有名詞なので**原則マスク**。
...
【要確認(unsure)】
- 展開や意味が分からず、固有名詞・自社固有名か、一般概念かを決められない略語。

確信度（confidence）＝「秘匿すべき固有の略語だ」とどれだけ確実に言えるか:
- high: 固有名詞（会社名・現地法人・組織・人名）または自社固有名だと明確。
- mid: 可能性は高いが確証がやや弱い。
- low: 手がかりが薄い。

最重要の原則:
- **固有名詞（会社名・現地法人名・組織名・人名）を指す略語は、製品名でなくても対象（原則マスク）**。
- 一方、**展開が一般的な概念の略語**（BSCK 等）や**公知の一般略語**（UI/API 等）は不要。
- 迷ったら yes に水増しせず unsure にする。
- reason には、英語名称（展開）・概要から「なぜ秘匿すべき固有／一般と判断したか」を具体的に書く。
...
def looks_like_abbrev(term: str) -> bool:
    """用語の表層が「略語・独自の短縮記号らしい」か（重点レビュー用のヒント。判定そのものではない）。

    - 空白を含む語句（``1st column`` 等）は句なので除外＝False。
    - ASCII 主体の短いトークン（区切り込みで概ね 10 文字以内）で、
      大文字か数字を含む（``AM`` ``ALGO-M`` ``BUF7`` ``FB``）ものを略語らしいとみなす。
    - 純粋な小文字語（``list`` ``value``）は含めない（普通の単語のことが多い）。
    """
    t = term.strip()
    if not t or " " in t or "　" in t:
        return False
    if _ABBREV_RE.fullmatch(t) is None:  # CJK・記号主体などは対象外（ASCII 略語に絞る）
        return False
    if re.fullmatch(
        r"[A-Z][a-z]+", t
    ):  # Titlecase 単語（Sony/Canon）＝普通の固有名詞語で略語でない
        return False
    core = re.sub(r"[-_.]", "", t)
    if len(core) > 10:
        return False
    return bool(re.search(r"[A-Z0-9]", t))
