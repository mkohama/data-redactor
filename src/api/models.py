...
class DocumentIngestRequest(BaseModel):
    """``POST /documents``（application/json）＝テキスト or kb-mcp 文書を取り込む。

    ``text`` か ``kb_doc_id`` のどちらか一方を指定する（バイナリは multipart で ``file``）。
    ``kb_doc_id`` 指定時は kb-mcp（``kb_url`` 省略で既定 URL）からチャンクを取得して取り込む。
    """
...
# --------------------------------------------------------------------------- #
# 全体面：/kb/documents（設計 §2-B・入力元選択）。kb-mcp の文書一覧をそのまま返す。
# --------------------------------------------------------------------------- #
class KbListResponse(BaseModel):
    """``GET /kb/documents`` のレスポンス（kb-mcp の文書 dict をそのまま並べる）。"""
