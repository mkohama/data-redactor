...
（例: ``src.llm`` の ``LLM_DETECT_*`` / ``LLM_WINDOW_*`` / ``RESOURCE_NAME_GPT41_MINI`` など）。
したがって .env は**どの src サブモジュールより先に** os.environ へ流し込む必要がある。
``import src.X`` は必ず本 ``src/__init__.py`` を先に実行するので、ここに置けば全入口
（app.py / cli.py / テスト）で確実に効く。

以前は ``src.sources.kb_mcp`` の ``load_dotenv`` に依存していたが、それは app.py で
engine 等より**後**に import されるため、.env の値が import 時読みに間に合わず効かなかった
（.env に書いた設定が反映されない不具合）。それをここで根治する。
