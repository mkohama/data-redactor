...
from src.masking.dictionary import contains_partial, normalize, normalize_cs
...
def _match_key(surface: str) -> str:
    """除外照合のキー（大小無視）：前後 strip ＋連続空白を 1 個に畳んでから正規化（NFKC+casefold）。
...
def _match_key_cs(surface: str) -> str:
    """大小区別の完全一致キー（NFKC のみ・casefold しない）。``case_sensitive`` 語用。"""
    return normalize_cs(_WHITESPACE.sub(" ", surface).strip())
...
class MaskAllowlist:
    ...
    def __init__(self, entries: Iterable[str | dict]) -> None:
        self._norm: set[str] = set()  # 完全一致キー（大小無視）
        self._partial: set[str] = set()  # 境界内包キー（大小無視・部分一致: true）
        self._cs_norm: set[str] = set()  # 完全一致キー（大小区別）
        self._cs_partial: set[str] = set()  # 境界内包キー（大小区別・部分一致: true）
        for e in entries:
            ...
            if not s:
                continue
            if cs:
                self._cs_norm.add(_match_key_cs(s))
                if partial:
                    self._cs_partial.add(normalize_cs(s))
            else:
    ...
    def matches(
        ...
        照合ロジックは辞書 :func:`dictionary.contains_partial` と共有。大小無視／大小区別
        （``case_sensitive: true``）の両方を見る。
        """
        if _match_key(surface) in self._norm:
            return True
        if self._cs_norm and _match_key_cs(surface) in self._cs_norm:
            return True
        if not self._partial and not self._cs_partial:
            return False
        surfaces = list(token_surfaces) if token_surfaces else [surface]
        return contains_partial(surfaces, self._partial, self._cs_partial)

    def __contains__(self, surface: str) -> bool:
        """完全一致のみ（後方互換）。内包も含めた判定は :meth:`matches`。"""
        return (
            _match_key(surface) in self._norm or _match_key_cs(surface) in self._cs_norm
        )

    def __bool__(self) -> bool:
        return bool(self._norm or self._cs_norm)

    def __len__(self) -> int:
        return len(self._norm) + len(self._cs_norm)
