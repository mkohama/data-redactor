...
呼べるメソッド（詳しい引数と戻り値は各メソッドの説明を参照）:
    ...
    文書管理（取込済み文書を content_hash で使い回す。全体面）:
        ...
        list_kb_documents(url=) -> list        kb-mcp の文書一覧。
        ingest_kb_document(doc_id, url=, source_name=)  kb-mcp 文書を取り込む。
...
class MaskClient:
    ...
    def list_kb_documents(self, *, url: str | None = None) -> list[dict[str, Any]]:
        """kb-mcp の文書一覧（GET /kb/documents）。url 省略でサーバ既定の kb-mcp。"""
        params = {"url": url} if url is not None else None
        resp = self._client.get("/kb/documents", params=params)
        if not resp.is_success:
            self._raise(resp)
        return resp.json()["documents"]

    def ingest_kb_document(
        self, doc_id: str, *, url: str | None = None, source_name: str | None = None
    ) -> dict[str, Any]:
        """kb-mcp の文書を取り込み content_hash を発行する（POST /documents）。"""
        payload: dict[str, Any] = {"kb_doc_id": doc_id}
        if url is not None:
            payload["kb_url"] = url
        if source_name is not None:
            payload["source_name"] = source_name
