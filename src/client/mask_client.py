...
data-redactor serve で起動したサーバ (既定 http://127.0.0.1:8509) の
...
呼べるメソッド (詳しい引数と戻り値は各メソッドの説明を参照):
    ...
    MaskClient(base_url="http://127.0.0.1:8509", timeout=120.0)
    ...
    文書管理 (取り込んだ文書を content_hash で指して使い回す):
...
使い方の例:
    ...
    with MaskClient() as client:
        ...
        # masked_parts は入力と同じ順・同じ id。
        # masked_text は機密を伏せ字にした本文 (例: SONY を [社1] に置換)。
        # これを LLM に渡す (原文は渡さない)。
...
class MaskClient:
    ...
    def __init__(
        self,
        base_url: str = "http://127.0.0.1:8509",
        *,
        ...
        base_url   サーバの URL (末尾の / は自動で除く)。既定 http://127.0.0.1:8509。
    ...
    def ingest_document(
        ...
        *,
        ...
        source_name は一覧表示用の任意メタ (file のとき省略するとアップロード名を使う。
        kb-mcp から取得した元ファイルなどは、ここに文書名を渡して一覧を分かりやすくできる)。
        ...
        data = {"source_name": source_name} if source_name is not None else None
        return self._unwrap(
            self._client.post(
                "/documents", data=data, files={"file": (name, blob, ctype)}
            )
        )
